...
test("Reusing Data Between API Calls", async ({ request }) => {
  ...
  const registrerBody = await registerResponse.json();
  ...
  expect(registrerBody.userId, "registerBody.userId is defined").toBeDefined();

  const userId = registrerBody.userId;
